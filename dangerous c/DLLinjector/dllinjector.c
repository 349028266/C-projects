...
int main(void){
    ...
    do {
        if (strcmp(pe32.szExeFile, "Notepad.exe") == 0) {
            ...
            if (hProcess == NULL) {
                printf("\033[0;31m(-) Failed to open target process %lu\n\033[0m", GetLastError());
                CloseHandle(Snapshot);
                return 1;
            }
            ...
            DWORD dwRet = GetCurrentDirectory(MAX_PATH, pathbuffer);

            char directory[MAX_PATH];
            strcpy(directory, pathbuffer);
            strcat(directory, "\\injection.dll");


            printf("\033[1;32m(+) Current directory is: %s\033[0m\n", directory);

            printf("\033[1;32m(+) Allocating memory in target process\033[0m\n");

            LPVOID memory = VirtualAllocEx(hProcess, NULL, sizeof(directory)+1, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
            ...
            WriteProcessMemory(hProcess, memory, &directory, sizeof(directory)+1, NULL);
